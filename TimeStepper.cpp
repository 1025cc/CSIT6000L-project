...
void RK4::takeStep(ParticleSystem* particleSystem, float stepSize) {
    ...
    // k2
    vector<Vector3f> k2Input;
    for (int i = 0; i < oldState.size(); ++i) {
        k2Input.push_back(oldState[i] + 0.5 * k1[i]);
    }
    vector<Vector3f> k2 = particleSystem->evalF(k2Input);
    for (int i = 0; i < oldState.size(); ++i) {
        k2[i] *= stepSize;
    }

    //k3
    vector<Vector3f> k3Input;
    for (int i = 0; i < oldState.size(); ++i) {
        k3Input.push_back(oldState[i] + 0.5 * k2[i]);
    }
    vector<Vector3f> k3 = particleSystem->evalF(k3Input);
    for (int i = 0; i < oldState.size(); ++i) {
        k3[i] *= stepSize;
    }

    // k4
    vector<Vector3f> k4Input;
    for (int i = 0; i < oldState.size(); ++i) {
        k4Input.push_back(oldState[i] + k3[i]);
    }
    vector<Vector3f> k4 = particleSystem->evalF(k4Input);
    for (int i = 0; i < oldState.size(); ++i) {
        k4[i] *= stepSize;
    }
    ...
    vector<Vector3f> newState;
    for (int i = 0; i < oldState.size(); ++i) {
        newState.push_back(oldState[i] + (1.0 / 6.0) * (k1[i] + 2 * k2[i] + 2 * k3[i] + k4[i]));
    }
...
void RKF45::takeStep(ParticleSystem *particleSystem, float stepSize) {
    ...
    do {
        ...
        vector<Vector3f> k2 = particleSystem->evalF(k2State);
        for (int i = 0; i < stateSize; ++i) {
            k2[i] *= step;
        }
        ...
        vector<Vector3f> k3 = particleSystem->evalF(k3State);
        for (int i = 0; i < stateSize; ++i) {
            k3[i] *= step;
        }
        ...
        //error = zk+1-yk+1
        vector<Vector3f> xRKF4;
        vector<Vector3f> xRKF5;
        vector<Vector3f> error;
